...
import type { DetectWorkerRequest, DetectWorkerResponse } from './detectWorker.js'
...
export async function detectSurfacesAsync(
  ...
  try {
    ...
    const { detectSurfaces } = await import('./surfaceDetect.js')
...
export async function detectMeshSurfacesAsync(
  ...
  try {
    ...
    // Fallback: run on main thread
    const { detectMeshSurfaces } = await import('./meshSurfaceDetect.js')
