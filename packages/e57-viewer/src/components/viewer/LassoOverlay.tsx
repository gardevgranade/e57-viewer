...
export default function LassoOverlay() {
  ...
  const hasSelection = lassoTriangleMode
    ? (lassoSelectedTriangles !== undefined && lassoSelectedTriangles !== null)
    : (lassoSelectedIds !== undefined && lassoSelectedIds !== null)
  ...
  function doSeparateTriangles() {
    ...
    for (const { surfaceId, triangleIndices } of lassoSelectedTriangles) {
      ...
      triangleIndices.forEach((ti, ni) => { for (let j = 0; j < 9; j++) selWt[ni * 9 + j] = (surf.worldTriangles as Float32Array)[ti * 9 + j] })
      addSurface({
  ...
  const W = 215
  const popX = popupPos ? Math.min(Math.max(popupPos.x, 8), globalThis.innerWidth - W - 8) : 0
  const popY = popupPos ? Math.min(Math.max(popupPos.y - 20, 8), globalThis.innerHeight - 280) : 0
      ...
      {hasSelection && popupPos && (
        <div ref={popupRef} style={{
          ...
          <div style={{ padding: '8px 12px', borderBottom: '1px solid rgba(255,255,255,0.07)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <button type="button" onClick={clearSelection} style={{ background: 'none', border: 'none', color: '#475569', cursor: 'pointer', fontSize: 14, padding: 0 }}>✕</button>
          </div>
                ...
                {showGroupMenu && (
                  <div style={{ paddingLeft: 28, paddingBottom: 4 }}>
                    {surfaceGroups.length === 0
                      ...
                      : surfaceGroups.map(g => (
                        <button type="button" key={g.id} onClick={() => doMoveToGroup(g.id)}
