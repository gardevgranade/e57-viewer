...
function fmtArea(m2: number) {
  ...
  if (m2 < 10_000) return `${m2.toFixed(2)} m²`
...
function flattenGroups(groups: SurfaceGroup[], parentId: string | null = null, depth = 0): { group: SurfaceGroup; depth: number }[] {
  const children = groups.filter(g => g.parentId === parentId)
  const result: { group: SurfaceGroup; depth: number }[] = []
...
export default function SurfaceTooltip() {
  ...
  useEffect(() => {
    ...
    globalThis.addEventListener('mousedown', onDown, true)
    return () => globalThis.removeEventListener('mousedown', onDown, true)
  ...
  const param = type === 'roof'
    ...
    : (type === 'floor' && surf.worldTriangles
      ? (() => {
          ...
          let sum = 0
          for (let i = 0; i < n; i++) sum += (surf.worldTriangles[i*9+1] ?? 0)
  ...
  const left = pos
    ? Math.min(pos.x + 14, globalThis.innerWidth - TW - 8)
    : Math.round(globalThis.innerWidth / 2 - TW / 2)
  const top = pos
    ? Math.min(pos.y - 10, globalThis.innerHeight - 480)
    : 80
  ...
  function handleSplit() {
    ...
    const replacements: PickedSurface[] = parts.map((wt, i) => {
      ...
      return {
        ...
        color: SPLIT_COLORS[i % SPLIT_COLORS.length] ?? '#888888', visible: surf.visible,
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        {/* Color picker */}
        <label style={{ cursor: 'pointer', flexShrink: 0, position: 'relative' }} title="Change color" aria-label="Change color">
          <div style={{
        ...
        {/* Close */}
        <button type="button" onClick={() => setSelectedSurfaceId(null)}
      ...
      <div style={{ display: 'flex', gap: 5 }}>
        <button type="button" style={BTN} onClick={() => updateSurface(surf.id, { visible: !surf.visible })}>
          {surf.visible ? '👁 Hide' : '🙈 Show'}
        </button>
        {surfType && (['Wall', 'Roof', 'Floor'] as const)
          .filter(t => t !== surfType)
          .map(t => (
            <button type="button" key={t} style={BTN}
      ...
      {surf.worldTriangles && surf.worldTriangles.length >= 27 && (
        <div style={{ display: 'flex', gap: 5 }}>
          <button type="button" style={{ ...BTN, color: '#7dd3fc' }} onClick={handleTrace}>📏 Measure</button>
          <button type="button" style={BTN} onClick={handleSplit}>✂️ Split</button>
        </div>
      )}
