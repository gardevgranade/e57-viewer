...
const ACCEPTED_EXTENSIONS = ['.e57', '.dae', '.obj', '.skp', '.dxf', '.dwg', '.ply', '.glb', '.gltf']
...
export default function DragDropZone() {
  const { endpoints, features } = useConfig()
  const {
    streamStatus,
    fileName,
    fileSize,
  ...
  const handleFile = useCallback(
    ...
    [reset, setUploading, setJobId, setStreamStatus, setFileType, setError, endpoints],
  )
  ...
  const handleAddressImport = useCallback(async () => {
    ...
    reset()
    try {
      if (!endpoints.addressImport) {
        setAddressError('Address import endpoint not configured')
        setAddressLoading(false)
        return
      }
      const res = await fetch(endpoints.addressImport, {
        method: 'POST',
      ...
      setAddressError(error instanceof Error ? error.message : 'Address import failed')
    }
    setAddressLoading(false)
  }, [addresses, radius, addressFilter, reset, setUploading, setFileType, setJobId, setStreamStatus, endpoints])
  ...
  const onMtlChange = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const companionUrl = endpoints.companion ? `${endpoints.companion}/${jobId}` : null
      if (!companionUrl) return
      const form = new FormData()
      form.append('mtl', file)
      const res = await fetch(companionUrl, { method: 'POST', body: form })
      const json = await res.json()
      ...
      console.error('[DragDrop] MTL upload error:', error)
    }
    setIsAddingCompanion(false)
  }, [jobId, incrementModelVersion, endpoints])
  ...
  const onTextureChange = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      console.error('[DragDrop] Texture upload error:', error)
    }
    setIsAddingCompanion(false)
  }, [jobId, incrementModelVersion, requiredTextures, endpoints])
  ...
  const fetchJobInfo = useCallback(async () => {
    if (!jobId) return
    try {
      const modelUrl = endpoints.model ? `${endpoints.model}/${jobId}` : null
      if (!modelUrl) return
      const res = await fetch(`${modelUrl}?info=1`)
      if (res.ok) {
  ...
  if (!features.fileUpload) return null
  ...
  return (
    <div className="w-full space-y-4">
      ...
      {mode === 'file' || !features.addressImport ? (
        ...
        <button
          ...
          className={[
            'flex w-full cursor-pointer flex-col items-center justify-center gap-3 rounded-2xl border-2 border-dashed p-10 text-center transition-all select-none',
            isDragging
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 p-6 space-y-4">
          <div className="space-y-2">
            <label htmlFor="address-0" className="text-xs font-medium text-white/60">Addresses</label>
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              <label htmlFor="radius-input" className="text-xs text-white/50">Radius (m)</label>
              <input
                id="radius-input"
                type="number"
                value={radius}
