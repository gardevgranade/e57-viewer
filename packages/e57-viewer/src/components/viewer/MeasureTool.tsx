...
import type { PickedSurface, SavedMeasurement } from '../../lib/viewerState'
...
function dist3(a: MeasurePoint | undefined, b: MeasurePoint | undefined) {
  if (!a || !b) return 0
  return Math.hypot(b.x - a.x, b.y - a.y, b.z - a.z)
}

function mid3(a: MeasurePoint | undefined, b: MeasurePoint | undefined): [number, number, number] {
  if (!a || !b) return [0, 0, 0]
    ...
    for (let i = 0; i < triCount; i++) {
      ...
      for (let j = 0; j < 3; j++) {
        ...
        if (existing) {
          ...
          edgeMap.set(ek, { v1: a.clone(), v2: b.clone(), count: 1 })
        }
      }
    }
...
function SavedMeasurementView({ m, dotRadius, onDelete, onContinue, onUpdatePoints, fmt, fmtArea, highlightedSegIdx, measureActive, flyCameraRef, measureSnap, snapCandidates, allMeasurements }: {
  ...
  useEffect(() => {
    ...
    globalThis.addEventListener('pointerdown', onClick)
    return () => globalThis.removeEventListener('pointerdown', onClick)
  }, [showMenu])

  // Drag handler for saved measurement points
  useEffect(() => {
    if ((draggingIdx === undefined || draggingIdx === null)) return
  ...
  const closingDist = m.isClosed && lastPtForClosing ? dist3(lastPtForClosing, pts[0]) : 0
  ...
  return (
    <>
      ...
      {m.isClosed && pts.length >= 3 && (() => {
        ...
        const closePt = pts.at(-1)
        if (!closePt) return null
        return (
          <Line
            points={[[closePt.x, closePt.y, closePt.z], [pts[0].x, pts[0].y, pts[0].z]]}
            color={segHighlighted ? lineHighColor : lineColor}
            lineWidth={segHighlighted ? 4 : 2}
            depthTest={false}
            dashed={isCutout}
      ...
      {pts.map((p, i) => {
        ...
        return (
          <mesh
            ...
            onPointerLeave={() => {
              if ((draggingIdx === undefined || draggingIdx === null)) { setHovered(null); gl.domElement.style.cursor = '' }
            }}
            onPointerOut={() => {
              if ((draggingIdx === undefined || draggingIdx === null)) { setHovered(h => h === i ? null : h); gl.domElement.style.cursor = '' }
            }}
            onPointerDown={(e) => {
              if (!canDrag) return
              e.stopPropagation()
...
export default function MeasureTool({ flyCameraRef }: MeasureToolProps) {
  ...
  useEffect(() => {
    ...
    const onMove = (e: MouseEvent) => {
      if (draggingIdxRef.current !== undefined && draggingIdxRef.current !== null) return
      const rect = gl.domElement.getBoundingClientRect()
      const ndcX = ((e.clientX - rect.left) / rect.width) * 2 - 1
      const ndcY = -((e.clientY - rect.top) / rect.height) * 2 + 1
      rc.setFromCamera(new THREE.Vector2(ndcX, ndcY), camera)

      let resolved: { pos: THREE.Vector3; type: SnapType | 'free' } | null
  ...
  useEffect(() => {
    ...
    const onClick = (e: MouseEvent) => {
      ...
      if (hitSphereIdx !== undefined && hitSphereIdx !== null) {
      ...
      if (g) {
        ...
        if (points.length >= 3) {
          const first = points[0]
          const d = Math.hypot(g.pos.x - first.x, g.pos.y - first.y, g.pos.z - first.z)
      ...
      if (hits.length > 0) {
        ...
        if (points.length >= 3) {
          const first = points[0]
          const d = Math.hypot(p.x - first.x, p.y - first.y, p.z - first.z)
          const closeThreshold = dotRadiusRef.current * 5
          if (d < closeThreshold) {
            setIsClosed(true)
            return
  ...
  const totalDist = points.length >= 2
    ...
    : 0
  const lastPtForClosing = points.at(-1)
  const closingDist = isClosed && lastPtForClosing ? dist3(lastPtForClosing, points[0]) : 0
  const perimeter = totalDist + closingDist
  ...
  return (
    <>
      ...
      {ghost && !isClosed && measureActive && points.length > 0 && (() => {
        ...
        const prevPt = hasPrev ? points.at(-2) : null
      ...
      {points.map((p, i) => {
        ...
        return (
          <mesh
            ...
            onPointerLeave={() => { if ((draggingIdx === undefined || draggingIdx === null)) setHoveredIdx(null) }}
            onPointerDown={e => {
              e.stopPropagation()
