...
export function detectSurfaces(
  ...
  for (let i = 0; i < count; i++) {
    const x = (worldPos[i * 3] ?? 0)
    const y = (worldPos[i * 3 + 1] ?? 0)
    const z = (worldPos[i * 3 + 2] ?? 0)
  ...
  for (;;) {
    ...
    for (let iter = 0; iter < 150; iter++) {
      ...
      const i0 = (pool[Math.floor(Math.random() * pool.length)] ?? 0)
      let i1 = (pool[Math.floor(Math.random() * pool.length)] ?? 0)
      let i2 = (pool[Math.floor(Math.random() * pool.length)] ?? 0)
      if (i1 === i0) i1 = (pool[(pool.indexOf(i0) + 1) % pool.length] ?? 0)
      if (i2 === i0 || i2 === i1) i2 = (pool[(pool.indexOf(i1) + 1) % pool.length] ?? 0)

      const ax = (worldPos[i0 * 3] ?? 0), ay = (worldPos[i0 * 3 + 1] ?? 0), az = (worldPos[i0 * 3 + 2] ?? 0)
      const bx = (worldPos[i1 * 3] ?? 0), by = (worldPos[i1 * 3 + 1] ?? 0), bz = (worldPos[i1 * 3 + 2] ?? 0)
      const cx = (worldPos[i2 * 3] ?? 0), cy = (worldPos[i2 * 3 + 1] ?? 0), cz = (worldPos[i2 * 3 + 2] ?? 0)
      ...
      for (const idx of pool) {
        const px = (worldPos[idx * 3] ?? 0), py = (worldPos[idx * 3 + 1] ?? 0), pz = (worldPos[idx * 3 + 2] ?? 0)
  ...
  for (let i = 0; i < count; i++) {
    const px = (worldPos[i * 3] ?? 0), py = (worldPos[i * 3 + 1] ?? 0), pz = (worldPos[i * 3 + 2] ?? 0)
  ...
  const centroidYs = foundPlanes.map((_, pi) => {
    ...
    let sum = 0
    for (const idx of pts) sum += (worldPos[idx * 3 + 1] ?? 0)
    return sum / pts.length
  })

  const medianCentroidY = ([...centroidYs].sort((a: number, b: number) => a - b)[Math.floor(centroidYs.length / 2)] ?? 0)

  // Build surfaces with labels
  const labelCounts: Record<string, number> = {}
  const surfaces: DetectedSurface[] = foundPlanes.map((plane, pi) => {
    const pts = planePoints[pi] ?? []
    const centroidY = (centroidYs[pi] ?? 0)
    ...
    return {
      ...
      color: (SURFACE_COLORS[pi % SURFACE_COLORS.length] ?? '#888888'),
      visible: true,
      normalY,
      centroidY,
  ...
  for (const surf of surfaces) {
    const count = (labelCounts[surf.label] ?? 0)
    if (count > 1) {
