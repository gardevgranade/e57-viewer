...
export function parseDxfToThree(dxfText: string): THREE.Group {
  ...
  if (dxf.tables?.layer?.layers) {
    for (const [name, layer] of Object.entries(dxf.tables.layer.layers)) {
      if ((layer as any).color !== undefined && (layer as any).color !== null) {
  ...
  function getColor(entity: any): number {
    if (entity.color !== undefined && entity.color !== null && entity.color !== 256 && entity.color !== 0) {
      return aciToHex(entity.color)
    }
    if (entity.layer && layerColors.has(entity.layer)) return (layerColors.get(entity.layer) ?? DEFAULT_LINE_COLOR)
...
const DEFAULT_LINE_COLOR = 0x8c_b4_d4
...
function aciToHex(aci: number): number {
  const ACI_MAP: Record<number, number> = {
    0: 0x8c_b4_d4, // ByBlock — bright on dark bg
    1: 0xff_44_44, // Red
    2: 0xff_ff_44, // Yellow
    3: 0x44_ff_44, // Green
    4: 0x44_ff_ff, // Cyan
    5: 0x44_88_ff, // Blue (brighter)
    6: 0xff_44_ff, // Magenta
    7: 0xe0_e0_e0, // White/Black → light grey
    8: 0x99_99_99, // Dark grey → medium grey
    9: 0xcc_cc_cc, // Light grey
    10: 0xff_66_66,
    11: 0xff_aa_66,
    30: 0xff_77_44,
    40: 0xff_aa_00,
  ...
  if (ACI_MAP[aci] !== undefined) return ACI_MAP[aci] ?? DEFAULT_LINE_COLOR
