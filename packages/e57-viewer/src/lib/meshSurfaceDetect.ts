...
function spatialComponents(tris: TriData[], thresholdOverride?: number): Uint32Array {
  ...
  for (let i = 0; i < n; i++) {
    const ai = (order[i] ?? 0)
    const triAi = tris[ai]; if (!triAi) continue
    const { cx: ax, cy: ay, cz: az } = triAi
    for (let j = i + 1; j < n; j++) {
      const bj = (order[j] ?? 0)
      const triBj = tris[bj]; if (!triBj) continue
  ...
  for (const cl of normalClusters) {
    ...
    for (let c = 0; c < numComponents; c++) {
      ...
      for (let i = 0; i < cl.tris.length; i++) {
        ...
        const tri = cl.tris[i]; if (!tri) continue
        totalArea += tri.area
...
export function splitSurfaceTriangles(
  ...
  for (let t = 0; t < triCount; t++) {
    const ax = (worldTriangles[t*9] ?? 0), ay = (worldTriangles[t*9+1] ?? 0), az = (worldTriangles[t*9+2] ?? 0)
    const bx = (worldTriangles[t*9+3] ?? 0), by = (worldTriangles[t*9+4] ?? 0), bz = (worldTriangles[t*9+5] ?? 0)
    const cx = (worldTriangles[t*9+6] ?? 0), cy = (worldTriangles[t*9+7] ?? 0), cz = (worldTriangles[t*9+8] ?? 0)
    const e1x = bx-ax, e1y = by-ay, e1z = bz-az
    const e2x = cx-ax, e2y = cy-ay, e2z = cz-az
    const nrmX = e1y*e2z - e1z*e2y
    const nrmY = e1z*e2x - e1x*e2z
    const nrmZ = e1x*e2y - e1y*e2x
    const area = Math.hypot(nrmX, nrmY, nrmZ) / 2
    totalArea += area
    tris.push({
  ...
  for (let i = 0; i < tris.length; i++) {
    const bucketIdx = (assignment[i] ?? 0)
    const tri = tris[i]; if (!tri) continue
    const bucket = buckets[bucketIdx]; if (!bucket) continue
    bucket.push(...tri.pos)
  }
  return buckets
    .filter(b => b.length > 0)
